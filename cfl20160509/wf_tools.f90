...
! states on the torus for fractional quantum Hall calculations
...
module form_factor_m
integer, parameter :: dp = kind(1.0d0)
real (kind=dp), allocatable, save, target ::  form_factor(:,:)
complex (kind=dp) :: l1_ff,l2_ff
integer :: norb_ff
end module form_factor_m

subroutine setup_formfactor_table
  use form_factor_m
  implicit none
  real(kind=dp) :: norm
  call get_l(norb_ff,l1_ff,l2_ff)
! make form-factor table
  if(allocated(form_factor)) deallocate(form_factor)
  allocate (form_factor(norb_ff,norb_ff))
  write(6,'("setting up form-factor table")')
  call make_gaussian_form_factor(norb_ff,form_factor,norm)
  form_factor  = form_factor**2


  return
end subroutine setup_formfactor_table

subroutine get_form_factor(norb,l1,l2,form_factor_copy)
  use form_factor_m
  integer, intent(in) :: norb
  complex (kind=dp), intent(out) :: l1,l2
  real(kind=dp), intent(out) :: form_factor_copy(norb,norb)
  integer :: i, j

  if(norb /= norb_ff) then
     write(6,'("get_form_factor: norb mismatch",2i10)') norb, norb_ff
     stop
  endif
  ...
  form_factor_copy = form_factor

  return
end subroutine get_form_factor
...
subroutine make_quantum_distance_table_old(norb,s2)
implicit none
integer,  parameter :: dp = kind(1.0d0)
integer, intent(in) :: norb
...
return
end subroutine make_quantum_distance_table_old
...
subroutine make_quantum_distance_table (norb, s2)
  ...
  integer, intent(in) :: norb
  real (kind=dp), intent(out) :: s2(norb,norb)
  real (kind=dp), pointer :: form_factor(:,:)
  complex (kind=dp) :: l1,l2
  integer :: n, m, i, j
  real (kind=dp), allocatable :: s2_temp(:,:,:,:)
  complex (kind=dp), allocatable :: omega(:)

  allocate (s2_temp(norb,norb,norb,1),form_factor(norb,norb),omega(0:norb-1))  
...
subroutine make_gaussian_form_factor(norb,u,norm)
  ...
  if(norb_z /= norb) then
     write(6,'("COMPACIFIED GAUSSIAN: norb mismatch",2i5)') norb, norb_z
  endif
...
subroutine make_lattice_landau_state(norb,k,sl2z,landau,nzeroes,k1,k2)
  ...
  integer, parameter :: bigint = 10000000
